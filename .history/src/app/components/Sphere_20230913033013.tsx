"use client";
import { OrbitControls, useRaycaster } from "@react-three/drei";
import { Canvas, useFrame, useThree,useRaycaster } from "@react-three/fiber";
  ...
  useFrame(() => {
    ...
    if (intersects.length > 0) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (distance < 0.1) {
          positionArray[i * 3] +=
            (x - intersects[0].point.x) * sensitivityFactor;
          positionArray[i * 3 + 1] +=
            (y - intersects[0].point.y) * sensitivityFactor;
          positionArray[i * 3 + 2] +=
            (z - intersects[0].point.z) * sensitivityFactor;
        }
      }
