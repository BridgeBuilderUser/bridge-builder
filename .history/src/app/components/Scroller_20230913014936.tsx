...
export default function Scroller({ children }: { children: React.ReactNode }) {
  ...
  const { setScrollDirection, setScrollPosition, setScrollPercent } = uiActions;
    ...
    ScrollSmoother.create({
      ...
      onUpdate: (self) => {
        if (self.scrollTrigger && track && bar) {
          ...
          barHeight(s);
            setScrollPosition(s);
            setScrollPercent(scrollTrigger.progress);
        }
      },
    });
  }, [scrollTrackRef, scrollBarRef, calcScrollDirection]);
