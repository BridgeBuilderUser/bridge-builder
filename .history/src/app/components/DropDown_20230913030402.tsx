...
export default function Dropdown({ children, dropdownContent }: { children: React.ReactNode, dropdownContent:any}) {
  return (
        ...
        <DD>
          ...
          <DropdownMenu aria-label="Static Actions" className="bg-white shadow-lg rounded-xl">
            {dropdownContent.map((item:any) => (
              <DropdownItem key={item.name} onClick={() => console.log(item.name)} className="text-black">
                {item.name}
              </DropdownItem>
            ))}
          </DropdownMenu>
