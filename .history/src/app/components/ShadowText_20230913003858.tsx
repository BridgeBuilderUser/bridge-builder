...
export default function ShadowText({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const split = () => {
      const splitText = new SplitText(ref.current, { type: "chars, words", position: "absolute", charsClass: "char", wordsClass: "word" });
      const chars = splitText.chars;
      const words = splitText.words;

      gsap.set(ref.current, { perspective: 400 });

      gsap.set(chars, { opacity: 0, yPercent: 100, transformOrigin: "0% 50%" });
      gsap.set(words, { opacity: 0, yPercent: 100, transformOrigin: "0% 50%" });
