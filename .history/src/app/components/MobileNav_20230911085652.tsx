...
export default function MobileNav({
  ...
  return (
    <>
      ...
      />
      <ul className={`mobile fixed h-screen w-screen top-0 left-0 flex h-full w-full flex-col items-center justify-center gap-5 lg:flex-row ${mobileNavOpen ? "open" : "closed"}`}>
        {links.map((link:any) => (
          <li key={link.name}>
