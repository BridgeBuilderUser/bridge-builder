...
export default function Scroller({ children }: { children: React.ReactNode }) {
  ...
  const gHeight =(h) => gsap.quickTo(scrollBarRef.current, { height: h, duration: 0.1, ease: "power2.out" })
  useLayoutEffect(() => {
  ...
  useEffect(() => {
    ...
    if (track && bar) {
      ...
      const scrollBarHeight = trackHeight * (trackHeight / track.scrollHeight);
      ...
      ScrollSmoother.create({
        ...
        onUpdate: (self) => {
          if (self.scrollTrigger) {
            console.log(self.scrollTrigger.progress);
            if (scrollTrackRef.current && scrollBarRef.current) {
              ...
              const updatedScrollBarHeight =
                ...
                (updatedTrack.clientHeight / updatedTrack.scrollHeight);

              gHeight(updatedScrollBarHeight * self.progress)
              // gsap.to(updatedBar, {
