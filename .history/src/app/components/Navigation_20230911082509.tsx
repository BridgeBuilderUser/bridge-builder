  ...
  return (
    <header className="flex items-center justify-center p-3 sticky top-0 z-50 bg-white shadow-md w-full">
      ...
      <GiHamburgerMenu
            ...
            className={`${isOpen ? 'hidden' : 'visible'} absolute top-5 right-5 h-5 w-5 text-black cursor-pointer`}
          />
          <AiOutlineCloseCircle
            onClick={() => setIsOpen(false)}
            className={`${isOpen ? 'visible' : 'hidden'} absolute top-5 right-5 h-5 w-5 text-black cursor-pointer`}
          />
    </header>
  );
}
