...
function calculateMaxFontSize(textElement: HTMLDivElement) {
  ...
  while (!wordFits) {
    ...
    console.log(textElement.offsetWidth, window.innerHeight)
    if (textElement.offsetWidth <= window.innerHeight) {
      fontSize++;
    } else {
      wordFits = true;
      fontSize--;
      gsap.set(textElement, { opacity: 1, writingMode: "vertical-rl"});
    }
  }
...
export default function ShadowText({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div ref={ref} className={`shadow-text top-0 left-[-10%] z-[10000000] font-montserrat-black font-black fixed w-auto h-auto   bg-gradient-to-r from-transparent from-20% to-indigo-800 bg-clip-text text-transparent break-words whitespace-nowrap`}>
      {children}
    </div>
  );
}
