...
export default function Dropdown({ children, dropdownContent }: { children: React.ReactNode, dropdownContent:any}) {
  return (
        ...
        <DD>
          ...
          <DropdownMenu aria-label="Static Actions" className="bg-white shadow-lg rounded-xl p-5">
            {dropdownContent.map((item:any) => (
              <DropdownItem key={item.name}  className="text-black border-none outline-none">
                {item.name}
              </DropdownItem>
            ))}
          </DropdownMenu>
